---
schema_version: 1
id: 01M30QZJQ5F06Z6W8N8M7DC4FA
key: SD-12
type: docs
title: "Icon: diamond at 70% fit box (icon rulings 2026-09-21)"
created_by: kevin
created_at: 2026-09-21T01:06:12Z
---

## Scope

`tools/icon.py`'s fit box for the vanilla diamond, per Kevin's icon rulings (2026-09-21):
pixel-art items sit at 70% of the previous fit box. `docs/modrinth/icon.png` is regenerated from
the new box; this mod stays on the Create blueprint-grid badge (it is a Create Fly add-on), so the
grid itself is untouched -- only the diamond's size changes. Not the badge colours, the
outline/shadow treatment, or anything else in `docs/modrinth/`.
...
(`diamond @ 70% (224px box)` in the icon-variants contact sheet Kevin reviewed).

## Acceptance criteria

- [x] `tools/icon.py`'s diamond fit box is 70% of its previous value (320 -> 224px), grid theme
      unchanged.
...
- [x] Committed on `documentation/sd-12-icon-size-70` and pushed; no PR, no `kontor finish`.
