...
vault_spec := env("SD_VAULT_SPEC", main_checkout / ".." / "heimathafen" / "vault" / "projects" / "create_synthetic_diamonds" / "spec")
...
# Render the Modrinth icon on the cubealgos navy badge (SD-6).
# Renders the vanilla diamond item sprite, read from the Minecraft client jar in the Gradle
# cache -- this mod adds no block or item of its own to draw instead (00-context.md). SD-6
# supplies tools/icon.py.
icon:
...
release:
    ...
    cp "build/libs/create_synthetic_diamonds-$version.jar" dist/
    (cd dist && shasum -a 256 "create_synthetic_diamonds-$version.jar" > "create_synthetic_diamonds-$version.jar.sha256")
