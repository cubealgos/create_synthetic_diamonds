#!/usr/bin/env python3
"""Render docs/modrinth/icon.png: the vanilla diamond on the cubealgos navy badge Create add-ons
share.

This mod adds no block or item of its own (00-context.md, UI-REQ-001: "no new screen, no new
block, no new item"), so the icon's subject is the game's own diamond item sprite, read straight
out of the Minecraft client jar in the Gradle cache -- never vendored into this repo. The jar is
found by globbing `~/.gradle/caches/fabric-loom/minecraftMaven/net/minecraft/minecraft-merged-deobf/26.2/*.jar`
...
SHADOW = (20, 50, 90, 130)
SPRITE_ENTRY = "assets/minecraft/textures/item/diamond.png"
...
def main() -> None:
    ...
    parser.add_argument("--jar", type=Path, help="Minecraft jar to read the diamond sprite from")
